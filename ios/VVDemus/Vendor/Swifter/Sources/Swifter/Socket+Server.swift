...
extension Socket {
    ...
    public func acceptClientSocket() throws -> Socket {
        ...
        if clientSocket == -1 {
            // VVDemus fork fix: read errno into a local immediately, before anything else
            // (String allocation included) has a chance to overwrite it, and pass it up.
            // The accept loop cannot tell a transient failure from a fatal one without it.
            let code = errno
            throw SocketError.acceptFailed(code, String(cString: strerror(code)))
        }
        Socket.setNoSigPipe(clientSocket)
        // VVDemus fork fix: give every accepted connection a read deadline, so a peer that
        // goes away without closing cannot pin a thread from the global pool forever.
        Socket.setReceiveTimeout(clientSocket, seconds: Socket.receiveTimeout)
