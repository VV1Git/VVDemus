...
public func websocket(
    ...
    return { request in
        ...
        let protocolSessionClosure: ((Socket) -> Void) = { socket in
            ...
            func read() throws {
                // VVDemus fork fix: this was `while true`. A connection accepted moments
                // before the server stopped could sit here forever, holding a thread and a
                // descriptor long after the server it belonged to was gone — the socket is
                // only closed by `stop()` if `stop()` knew about it. The loop now notices
                // the socket being shut down underneath it and unwinds.
...
public class WebSocketSession: Hashable, Equatable {
    ...
    // VVDemus fork: `payloadTooLarge` added — see `maximumPayloadLength`.
    public enum WsError: Error {
        case unknownOpCode(String), unMaskedFrame(String), protocolError(String), invalidUTF8(String)
    ...
    /// Largest payload a single incoming frame may declare.
    ///
    /// VVDemus fork addition. The declared length is read off the wire and used to size an
    /// allocation before any of the payload exists, so it has to be bounded by something.
    /// The web remote's own frames are a few dozen bytes (`hello:<tab id>`); this is
    /// generous by many orders of magnitude and exists only to keep a hostile or broken
    /// client from choosing the app's memory usage.
    public static var maximumPayloadLength: UInt64 = 16 * 1024 * 1024
    ...
    public func readFrame() throws -> Frame {
        ...
        if len == 0x7E {
            ...
            // VVDemus fork fix: the first shift was `<< 54`, not `<< 56`, so every 64-bit
            // frame length whose top byte was set decoded to a quarter of its real value.
        ...
        // VVDemus fork fix: `Int(len)` below traps for anything above `Int.max`, so eight
        // 0xFF bytes from any client on the Wi-Fi killed the app outright — and short of
        // that, `read(length:)` was handed a client-chosen allocation size before a single
        // payload byte had arrived. Bounded here as well as in `Socket.read(length:)` so
        // the frame is rejected as a protocol error rather than a socket failure.
        guard len <= WebSocketSession.maximumPayloadLength else {
            throw WsError.payloadTooLarge(length: len, limit: WebSocketSession.maximumPayloadLength)
        }
        ...
        // VVDemus fork fix: was `for index in 0..<len`, indexing `frm.payload` by a count
        // taken from the frame header rather than by what was actually read — out of range
        // the moment those two disagree.
        for index in frm.payload.indices {
            frm.payload[index] ^= mask[index % 4]
        }
        return frm
    }
